...
import { FaCheck, FaSave, FaTimes, FaClock, FaChevronDown, FaChevronUp } from "react-icons/fa";
...
const AttendanceForm = ({ user, onSuccess, onClose }) => {
  ...
  useEffect(() => {
    const fetchStudentsAndAttendance = async () => {
      ...
      setLoading(true);
      try {
        console.log('Fetching data for class:', selectedClass, 'date:', selectedDate);
        ...
        console.log('Raw student data from API:', JSON.stringify(studentData, null, 2));
        console.log('Raw attendance data from API:', JSON.stringify(existingAttendance, null, 2));
        
        // Debug: Log the raw attendance data we received
        console.log('Raw attendance response:', {
          recordCount: Array.isArray(existingAttendance) ? existingAttendance.length : 'not an array',
        ...
        if (!Array.isArray(existingAttendance) || existingAttendance.length === 0) {
          console.log('No attendance records found for the specified date. Checking all records for this class...');
          try {
            ...
            console.log('All attendance records for class:', {
              ...
              dates: allClassRecords.data?.map(r => r.date).filter((v, i, a) => a.indexOf(v) === i),
        ...
        if (Array.isArray(existingAttendance) && existingAttendance.length > 0) {
          ...
          existingAttendance.forEach((record, index) => {
            ...
            if (record?.student_id) {
              ...
              const status = ['present', 'absent', 'late'].includes(record.status?.toLowerCase()) 
                ? record.status.toLowerCase() 
                : 'absent';
              
              console.log(`  Mapping - Student ID: ${studentId}, Status: ${status}`);
          ...
          console.log('Attendance map after processing:', Object.fromEntries(attendanceMap));
        } else {
          console.log('No attendance records found for this date or invalid format');
        }
        ...
        studentData.forEach((item, index) => {
          ...
          const studentId = (item?.student?.id || item?.id || item?.student_id)?.toString();
          
          if (!studentId) {
            console.warn(`Student ${index + 1}: Could not determine ID for student:`, item);
            return;
          }
        ...
        console.log('\nFinal attendance mapping:', JSON.stringify(initialAttendance, null, 2));
  ...
  const handleAttendanceChange = (studentId, status) => {
    ...
    console.log(`Updating attendance - Student ID: ${idString}, Status: ${status}`);
  ...
  const handleSaveAttendance = async () => {
    ...
    try {
      ...
      const updatePromises = attendanceRecords.map(record => {
        const existingRecord = existingAttendance?.find(a => a.student_id === record.student_id);
        if (existingRecord) {
  ...
  <div className="mb-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <div className="sm:col-span-2 lg:col-span-1 flex items-end">
      ...
      >
        {saving ? <FaSave className="mr-2" /> : <FaCheck className="mr-2" />}
